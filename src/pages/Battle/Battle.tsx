      ...
      >
        <BattleSectionContainer>
          <UserPokemon
            ...
            throwUserPokeball={throwUserPokeball}
          />
          <BotPokemon
            botAttackMove={botAttackMove}
            throwBotPokeball={throwBotPokeball}
          />
        </BattleSectionContainer>
        <ActionsSection>
          <InnerContainerBorder className="center_abs_item">
            ...
            {pauseRef.current ? (
              <AvailablePokemonList handleUserSelection={handleUserSelection} />
            ) : (
              <>
                <PixelatedButton
                  disabled={pauseRef.current}
                  onClick={() => handleUserAction("attack")}
                  className="fightBtn"
                  style={{ background: "#7b151e" }}
                >
                  {pauseRef.current ? "Wait" : "Fight"}
                </PixelatedButton>
                <PixelatedButton
                  disabled={pauseRef.current}
                  onClick={() => handleUserAction("change")}
                  className="changeBtn"
                  style={{ background: "#bd6917" }}
                >
                  {pauseRef.current ? "Wait" : "Change"}
                </PixelatedButton>
              </>
            )}
          </InnerContainerBorder>
