...
const BattleEnd = ({message}:IBattleEnd) => {
  ...
  return (
    <Box width='100%' height="100vh" zIndex={500} position='fixed' display="flex" alignItems='center' flexDirection='column' justifyContent='center' color='white' sx={{ background:"rgba(0,0,0,.85)"}} >

        <Typography fontSize={{xs:'28px',sm:"48px"}}> Battle has ended</Typography>
        <Typography fontSize={{xs:'36px',sm:"72px"}} fontWeight={600} color={color}> {message}</Typography>
        <Box position='absolute' bottom='10%' display='flex' alignItems='center' flexDirection='column' justifyContent='center'> 
            <Typography fontSize={{xs:'22px',sm:"30px"}} fontWeight={600} mb={1}> Thank you for playing</Typography>
