...
function UserPokemon({ attackMove, throwUserPokeball }: IUserPokemonProps) {
  const { userPokemon } = useSelector((store: AppStore) => store.game);
  const normalise = (currValue: number) =>
    Math.floor(100 * (currValue / userPokemon.health));
    ...
    <>
      ...
      <UserLifeBar>
        ...
        <Typography
          ...
          right="15%"
        >
          {userPokemon.currentHealth}/{userPokemon.health}
        </Typography>
      </UserLifeBar>
    </>
  );
