...
function PokemonGridCard({ pokemon }: IPokemonGridCard) {
  ...
  const normalise = (currValue: number) =>
    Math.floor(100 * (currValue / pokemon.health));
  const normalizedHealth = normalise(pokemon.currentHealth);
  const healthBarColors =
    normalizedHealth > 50
      ? "success"
      : normalizedHealth > 20 && normalizedHealth <= 50
      ? "warning"
      : "error";
        ...
        <GridItem>
          ...
          >
            ...
            >
              {status === "alive" ? (
                <>
                  <Typography fontSize={{ xs: "15px", sm: "17px" }}>
                    HP{" "}
                  </Typography>
                  <Typography
                    fontFamily="Yrsa"
                  ...
                  <LinearProgress
                    variant="determinate"
                    color={healthBarColors}
                    value={normalizedHealth}
                    sx={{ width: "80%", height: "15px", borderRadius: "10px" }}
                  />
                </>
              ) : (
                <Typography
